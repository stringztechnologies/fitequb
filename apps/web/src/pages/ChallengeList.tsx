...
export function ChallengeList() {
	...
	useEffect(() => {
		// Always use public endpoints — data is the same, no auth needed
		Promise.all([
			publicApi<Challenge[]>("/public/challenges"),
			publicApi<LeaderboardEntry[]>("/public/gamification/leaderboard"),
		])
			.then(([cRes, lRes]) => {
		...
		<div className="bg-surface min-h-screen pb-24 px-4 pt-5">
					...
					<div className="flex flex-col gap-3">
						{challenges.map((ch) => (
							<ChallengeCard
								...
								challenge={ch}
								onJoin={() =>
									isGuest ? showModal("join this challenge") : navigate(`/challenges/${ch.id}`)
								}
							/>
						))}
					</div>
			...
			{hasChallenges && (
				<button
					...
					onClick={() =>
						isGuest
							? showModal("join a challenge and log steps")
				...
				>
					{isGuest ? "JOIN A CHALLENGE" : "UPDATE MY STEPS"}
				</button>
			)}
...
function ChallengeCard({
	...
	const daysLeft = ch.end_date
		? Math.max(0, Math.ceil((new Date(ch.end_date).getTime() - Date.now()) / (1000 * 60 * 60 * 24)))
		: 0;
		...
		<div className="bg-surface-container-low rounded-lg p-5 border border-outline-variant/10">
			<div className="flex items-start gap-3.5">
				...
				<div className="flex-1 min-w-0">
					...
					<p className="text-on-surface-variant text-xs mb-3">{ch.description}</p>
