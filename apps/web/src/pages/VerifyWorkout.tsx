		...
		<div className="bg-background text-on-surface font-body min-h-screen pb-32">
			...
			<div className="px-5 pt-4 space-y-5">
				...
				{cards.map((card) => (
					<div
						key={card.key}
						className={`glass-card rounded-2xl p-5 ${card.done ? "opacity-60" : ""}`}
					>
						<div className="flex items-center justify-between mb-3">
							<div className="flex items-center gap-3">
								<div
									className={`w-10 h-10 rounded-full flex items-center justify-center ${card.done ? "bg-on-surface-variant/10" : "bg-primary/15"}`}
								>
									<span
										className={`material-symbols-outlined text-xl ${card.done ? "text-on-surface-variant" : "text-primary"}`}
								...
								<div>
									...
									<p
										className={`font-label text-2xs uppercase tracking-widest ${card.done ? "text-on-surface-variant" : "text-primary"}`}
									>
						...
						{!card.done && card.key === "steps" && (
							<div className="flex gap-2 mt-2">
								...
								<button
									type="button"
									onClick={submitSteps}
									disabled={!steps || activeMethod === "steps"}
						...
						{!card.done && card.key === "gps" && (
							<button
								type="button"
								onClick={submitGps}
								disabled={gpsLoading || activeMethod === "gps"}
								className="mt-2 w-full py-3 rounded-full bg-primary/15 text-primary font-headline text-sm font-bold active:scale-95 transition-transform disabled:opacity-30"
							>
