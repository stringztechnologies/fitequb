...
export function EqubList() {
	...
	useEffect(() => {
		...
		const path = isGuest ? "/public/equb-rooms" : "/api/equb-rooms";
		fetch<EqubRoom[]>(path)
			.then((res) => {
	...
	const totalPayout = rooms.reduce((sum, r) => sum + r.stake_amount * r.max_members, 0);
	...
	return (
		<div className="bg-background text-on-surface font-body min-h-screen pb-32">
			...
			<button
				type="button"
				onClick={() => (isGuest ? showModal("quick join an Equb") : navigate("/quick-join"))}
			...
			<button
				type="button"
				onClick={() => (isGuest ? showModal("create an Equb room") : navigate("/equbs/create"))}
