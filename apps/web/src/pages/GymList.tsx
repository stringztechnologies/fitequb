...
export function GymList() {
	...
	useEffect(() => {
		...
		const path = isGuest ? "/public/gyms" : "/api/gyms";
		fetch<PartnerGym[]>(path)
			.then((res) => {
	...
	return (
		<div className="bg-surface pb-24 min-h-screen">
			...
			<p className="text-xs font-body text-on-surface-variant px-5 pb-4">
				<span className="text-primary font-bold">Equb Eligible</span> = Check-ins count toward your
				Equb workout target
			</p>
				...
				{filteredGyms.length > 0 ? (
					filteredGyms.map((g) => (
						<RealGymCard
							...
							gym={g}
							isGuest={isGuest}
							onGuestAction={() => showModal("buy a day pass")}
						/>
					))
				) : (
					<EmptyState
			...
			<div className="p-5">
				...
				<div className="flex items-start justify-between mb-3">
					<div className="flex items-center gap-3">
						...
						<div>
							<h3 className="font-headline text-lg font-bold text-on-surface">{gym.name}</h3>
							<div className="flex items-center gap-1 mt-0.5">
								<span className="material-symbols-outlined text-on-surface-variant text-xs">
									location_on
								</span>
								<span className="text-xs font-body text-on-surface-variant">{gym.location}</span>
							</div>
						</div>
					</div>
				</div>
