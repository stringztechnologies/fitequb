    ...
    <div className="bg-background text-on-surface font-body pb-44 min-h-screen">
      ...
      <section className="px-5 pb-6">
        ...
        <button
          type="button"
          onClick={() => {
            const link = `https://t.me/fitequb_bot?start=DUEL-${Date.now()}`;
            const text =
              "Challenge me to a 1v1 fitness duel on FitEqub! 500 ETB each, winner takes all.";
            if (navigator.share) {
              navigator
                .share({ title: "FitEqub Duel", text, url: link })
                .catch(() => {});
            } else {
              window.open(
      ...
      <div className="fixed bottom-16 left-0 right-0 z-40 px-5 pb-4 pt-6 bg-gradient-to-t from-background via-background/95 to-transparent">
        <button
          ...
          onClick={() => {
            ...
            if (navigator.share) {
              navigator
                .share({ title: "FitEqub Duel Challenge", text })
                .catch(() => {});
