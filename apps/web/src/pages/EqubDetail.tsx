...
import {
  TelegramModal,
  useTelegramModal,
} from "../components/TelegramModal.js";
...
export function EqubDetail() {
  ...
  const daysLeft =
    room.status === "active" ? calculateDaysRemaining(room.end_date) : null;
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-surface-container-low p-5 rounded-lg border border-outline-variant/10">
            ...
            <p className="text-xs text-on-surface-variant">
              Complete {Math.ceil(target * pct)} of {target} in{" "}
              {room.duration_days} days ({Math.round(pct * 100)}%)
            </p>
          </div>
          <div className="bg-surface-container-low p-5 rounded-lg border border-outline-variant/10">
            <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center mb-3">
      ...
      <section className="px-4 mt-6">
        ...
        <div className="flex flex-col gap-3">
          {members.map((m, idx) => {
            ...
            const onTrack =
              memberPct >= (daysElapsed / room.duration_days) * 0.8;
      ...
      <section className="px-4 mt-6">
        <button
          ...
          onClick={() => {
            ...
            if (navigator.share) {
              navigator
                .share({ title: `FitEqub \u2014 ${room.name}`, text })
                .catch(() => {});
