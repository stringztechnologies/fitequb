	...
	return (
		<div className="min-h-screen bg-background text-on-surface font-body">
			...
			<main className="max-w-lg mx-auto px-6 py-8">
				...
				{/* Results */}
				<output className="block mt-8" aria-live="polite">
					{state.step === "valid" && <ValidCard pass={state.pass} onRedeem={redeem} />}
					{state.step === "redeemed" && <RedeemedCard pass={state.pass} onReset={reset} />}
					{state.step === "already_used" && <AlreadyUsedCard onReset={reset} />}
					{state.step === "invalid" && <InvalidCard message={state.message} onReset={reset} />}
					{state.step === "error" && <ErrorCard message={state.message} onReset={reset} />}
				</output>
			</main>
		</div>
	);
	...
	return (
		<div className="rounded-2xl border-2 border-secondary bg-secondary/10 p-6 space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
			<div className="flex items-center gap-3">
				...
				<span className="font-headline text-xl font-bold text-secondary">VALID PASS</span>
			</div>
			<dl className="grid grid-cols-2 gap-y-3 gap-x-4 text-sm">
				...
				<dt className="text-on-surface-variant font-label uppercase text-xs tracking-wider">Gym</dt>
				<dd className="text-on-surface font-bold">{pass.gym_name}</dd>
				<dt className="text-on-surface-variant font-label uppercase text-xs tracking-wider">
					Type
				</dt>
				<dd className="text-on-surface font-bold capitalize">{pass.pass_type}</dd>
				<dt className="text-on-surface-variant font-label uppercase text-xs tracking-wider">
					Expires in
				</dt>
...
function RedeemedCard({
	...
	return (
		<div className="rounded-2xl border-2 border-secondary bg-secondary/10 p-6 text-center space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
			...
			<p className="font-headline text-2xl font-bold text-secondary">REDEEMED</p>
...
function AlreadyUsedCard({ onReset }: { onReset: () => void }) {
	return (
		<div className="rounded-2xl border-2 border-[#F59E0B] bg-[#F59E0B]/10 p-6 text-center space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-300">
			...
			<p className="font-headline text-xl font-bold text-[#F59E0B]">ALREADY REDEEMED</p>
			<p className="text-sm text-on-surface-variant">
