...
trainers.post("/register", async (c) => {
	...
	const code = `FIT-${telegramUser.username ?? telegramUser.id}`.toUpperCase().slice(0, 20);
...
trainers.post("/request-payout", async (c) => {
	...
	if (payoutError) {
		return c.json<ApiResponse<null>>({ data: null, error: payoutError.message }, 500);
	}
	...
	if (success) {
		...
		await supabase.from("trainers").update({ pending_balance: 0 }).eq("id", trainer.id);

		await supabase.from("trainer_payouts").update({ status: "completed" }).eq("id", payout.id);
	} else {
		await supabase.from("trainer_payouts").update({ status: "failed" }).eq("id", payout.id);
	}
