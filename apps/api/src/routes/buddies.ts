...
import { z } from "zod";
import { notifyBuddyRequest } from "../lib/bot-notify.js";
...
buddies.post("/request", async (c) => {
  ...
  if (!members || members.length < 2) {
    return c.json(
      { data: null, error: "Both users must be in the same Equb" },
      400,
    );
  }
  ...
  const { data: existing } = await supabase
    ...
    .eq("equb_room_id", equb_id)
    .or(
      `and(user_id.eq.${userId},buddy_id.eq.${partner_user_id}),and(user_id.eq.${partner_user_id},buddy_id.eq.${userId})`,
    )
    .single();

  if (existing) {
    return c.json({
      data: { buddy_id: existing.id, status: existing.status },
      error: null,
    });
  }
  ...
  const { data: buddy, error } = await supabase
    .from("workout_buddies")
    .insert({
      equb_room_id: equb_id,
      user_id: userId,
      buddy_id: partner_user_id,
      status: "pending",
    })
    .select()
    .single();
  ...
  // Send Telegram notification to the partner
  const { data: requester } = await supabase
    .from("users")
    .select("full_name")
    .eq("id", userId)
    .single();
  const { data: equbRoom } = await supabase
    .from("equb_rooms")
    .select("name")
    .eq("id", equb_id)
    .single();
  ...
  notifyBuddyRequest(
    ...
    equbRoom?.name ?? "your Equb",
  ).catch(() => {});

  return c.json(
    { data: { buddy_id: buddy.id, status: "pending" }, error: null },
    201,
  );
});
...
buddies.post("/accept", async (c) => {
  ...
  if (error || !buddy) {
    return c.json(
      { data: null, error: "Buddy request not found or already accepted" },
      404,
    );
  }

  return c.json({
    data: { buddy_id: buddy.id, status: "active" },
    error: null,
  });
});
...
buddies.get("/my-buddy", async (c) => {
  ...
  const buddyUserId =
    pairing.user_id === userId ? pairing.buddy_id : pairing.user_id;
