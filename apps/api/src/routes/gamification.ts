...
gamification.get("/profile", async (c) => {
  ...
  const currentLevel =
    LEVEL_THRESHOLDS.find((l) => l.level === user.level) ?? LEVEL_THRESHOLDS[0];
...
gamification.post("/referral", async (c) => {
  ...
  if (!code) {
    return c.json<ApiResponse<null>>(
      { data: null, error: "Referral code is required" },
      400,
    );
  }
  ...
  if (!referrer) {
    return c.json<ApiResponse<null>>(
      { data: null, error: "Invalid referral code" },
      400,
    );
  }
  ...
  if (referrer.id === userId) {
    return c.json<ApiResponse<null>>(
      { data: null, error: "Cannot refer yourself" },
      400,
    );
  }
  ...
  if (existing) {
    return c.json<ApiResponse<null>>(
      { data: null, error: "Already used a referral code" },
      400,
    );
  }
