...
const createRoomSchema = z.object({
	...
	completion_pct: z.number().min(0.5).max(1.0).default(EQUB_DEFAULT_COMPLETION_PCT),
...
equbRooms.post("/", async (c) => {
	...
	return c.json<ApiResponse<EqubRoom>>({ data: room as EqubRoom, error: null }, 201);
});
...
equbRooms.post("/:id/join", async (c) => {
	...
	const { data: room } = await supabase.from("equb_rooms").select("*").eq("id", roomId).single();
	...
	if (count !== null && count >= room.max_members) {
		return c.json<ApiResponse<null>>({ data: null, error: "Room is full" }, 400);
	}
	...
	if (existing) {
		return c.json<ApiResponse<null>>({ data: null, error: "Already joined this room" }, 400);
	}
...
equbRooms.post("/quick-join", async (c) => {
	...
	if (!parsed.success) {
		return c.json<ApiResponse<null>>({ data: null, error: "Invalid tier" }, 400);
	}
...
equbRooms.post("/mark-result-seen", async (c) => {
	...
	if (!roomId) {
		return c.json<ApiResponse<null>>({ data: null, error: "room_id required" }, 400);
	}
