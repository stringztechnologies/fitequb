...
duels.post("/create", async (c) => {
  ...
  const { opponent_username, stake_amount, duration_days, daily_target } =
    parsed.data;
...
duels.post("/:id/accept", async (c) => {
  ...
  if (!room) {
    return c.json<ApiResponse<null>>(
      { data: null, error: "Duel not found" },
      404,
    );
  }
  ...
  if (room.created_by === userId) {
    return c.json<ApiResponse<null>>(
      { data: null, error: "Cannot accept your own duel" },
      400,
    );
  }
  ...
  if (existing) {
    return c.json<ApiResponse<null>>(
      { data: null, error: "Already joined this duel" },
      400,
    );
  }
  ...
  const txRef = `equb-${duelId}-${userId}-${Date.now()}`;
