import type { ApiResponse, Challenge, ChallengeParticipant } from "@fitequb/shared";
...
challenges.post("/:id/join", async (c) => {
	...
	if (existing) {
		return c.json<ApiResponse<null>>({ data: null, error: "Already joined" }, 400);
	}
...
challenges.post("/:id/log-steps", async (c) => {
	...
	if (!parsed.success) {
		return c.json<ApiResponse<null>>({ data: null, error: "Invalid step count" }, 400);
	}
	...
	if (error || !participant) {
		return c.json<ApiResponse<null>>({ data: null, error: "Not a participant" }, 400);
	}
