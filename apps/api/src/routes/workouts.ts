...
workouts.post("/", async (c) => {
  ...
  if (!membership) {
    return c.json<ApiResponse<null>>(
      { data: null, error: "Not a member of this room" },
      403,
    );
  }
  ...
  if (!room || room.status !== "active") {
    return c.json<ApiResponse<null>>(
      { data: null, error: "Room is not active" },
      400,
    );
  }
  ...
  if (existingWorkout) {
    return c.json<ApiResponse<null>>(
      { data: null, error: "Already logged a workout today" },
      400,
    );
  }
  ...
  checkStreakBadges(userId).catch(() => {});
  checkAndNotifyLevelUp(userId, pointsBefore).catch(() => {});

  return c.json<ApiResponse<Workout>>(
    { data: workout as Workout, error: null },
    201,
  );
});
