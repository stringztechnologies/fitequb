...
test.describe("Home Page", () => {
	...
	test("green ETB amount in progress ring (32px, #00C853)", async ({ page }) => {
		const amount = page.locator("text=ETB").first();
	...
	test("Potential Payout label is gold (#FFD700)", async ({ page }) => {
		const label = page.locator("text=Potential Payout");
		await expect(label).toBeVisible();
		const color = await label.evaluate((el) => getComputedStyle(el).color);
		expect(color).toBe(COLORS.gold);
	});

	test("FitEqub title is 36px", async ({ page }) => {
		const title = page.locator("h1");
		const fontSize = await title.evaluate((el) => getComputedStyle(el).fontSize);
		expect(fontSize).toBe("36px");
	});

	test("Your Progress is title case, not uppercase", async ({ page }) => {
		const text = await page.locator("text=Your Progress").textContent();
		expect(text).toBe("Your Progress");
	});

	test("feature cards have 48px gold icon boxes", async ({ page }) => {
		// Icon container with gold tint background
		const iconBox = page.locator("[style*='rgba(255,215,0,0.15)']").first();
		await expect(iconBox).toBeVisible();
		const size = await iconBox.evaluate((el) => getComputedStyle(el).width);
		expect(size).toBe("48px");
	});

	test("feature cards have green progress bars", async ({ page }) => {
		// Progress bars inside cards
		const bars = page.locator("[style*='backgroundColor'][style*='#00C853']");
		const count = await bars.count();
		expect(count).toBeGreaterThanOrEqual(3);
	});

	// Potential Payout already tested with gold color check above
...
test.describe("Notifications Page", () => {
	...
	test("notification cards have colored left borders", async ({ page }) => {
		const firstCard = page.locator("[class*='rounded']").filter({ hasText: "Payout" }).first();
		const borderLeft = await firstCard.evaluate((el) => getComputedStyle(el).borderLeftColor);
	...
	test("5 notification cards present", async ({ page }) => {
		const cards = page.locator("text=mins ago, text=hour ago, text=hours ago, text=Yesterday");
...
test.describe("Gym List Page", () => {
	...
	test("active filter chip has gold border", async ({ page }) => {
		...
		const borderColor = await nearMe.evaluate((el) => getComputedStyle(el).borderColor);
		expect(borderColor).toBe(COLORS.gold);
	});
});
...
test.describe("Sync Fitness Page", () => {
	...
	test("yellow Connect buttons", async ({ page }) => {
		...
		const bg = await connectBtn.evaluate((el) => getComputedStyle(el).backgroundColor);
